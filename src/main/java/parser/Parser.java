...
=> De rightChild van deze previousNode vervangen met nieuwe node: leftChild=previousNode.rightChild, operator (*), rightChild= nextNumber (6).
    ...
    public Node parse() {
        // Tree structure: each node consists of 3 elements: previous node-object(first one is the leftNumber, afterwards all expressionNodes), operator, rightNumber.
        ...
        for (int i = 1; i < tokens.size(); i += 2) {
            ...
            Node rightNode;
            // Brackets can be in the first e.g. (3-1) or third token 2*(3-1). Not 2nd as a * will be added beforehand during lexing, e.g. -*(5).
            // If first node is a bracket, for the subset of tokens within the brackets, create a subtree, set its firstNode as lastNode of the main tree.
            ...
            else if (tokens.get(i + 1).getType() == Token.Type.BRACKET) {
                List<Token> sublistOfTokens = sublistOfTokens(tokens.subList(i + 1, tokens.size()));
